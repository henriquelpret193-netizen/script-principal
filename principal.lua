...
AddToggle("X-RAY (VER ATRAVÉS DE PAREDES)", "XRay", Pages["ESP"])
...
local function setXRay(enable)
    if enable then
        for _, obj in ipairs(workspace:GetDescendants()) do
            if (obj:IsA("BasePart") or obj:IsA("MeshPart") or obj:IsA("UnionOperation") or obj:IsA("NegateOperation")) then
                
                -- Ignora personagens
                if not obj:FindFirstAncestorWhichIsA("Model") or not obj.Parent:FindFirstChild("Humanoid") then
...
-- ==================== CONTROLE DO X-RAY ====================
local lastXRay = false

RunService.RenderStepped:Connect(function()
    if _G.XRay ~= lastXRay then
        lastXRay = _G.XRay
        setXRay(_G.XRay)
    end
end)
...
local Player = Players.LocalPlayer
-- Certifica-se de encontrar ou criar a ScreenGui para a interface
local ScreenGui = Player:WaitForChild("PlayerGui"):FindFirstChild("FlyGui") or Instance.new("ScreenGui")
...
-- ==================== FLY TOGGLE GUI ====================
local FlyToggleGui = nil

local function createFloatingFlyToggle()
    if FlyToggleGui then FlyToggleGui:Destroy() end

    FlyToggleGui = Instance.new("Frame")
    FlyToggleGui.Name = "FlyByOhickToggle"
    FlyToggleGui.Size = UDim2.new(0, 190, 0, 85)
    FlyToggleGui.Position = UDim2.new(0.5, -95, 0.65, 0)
    FlyToggleGui.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    FlyToggleGui.BorderSizePixel = 0
    FlyToggleGui.ZIndex = 10000
    FlyToggleGui.Active = true
    FlyToggleGui.Parent = ScreenGui

    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 14)
    corner.Parent = FlyToggleGui
    ...
    ToggleButton.Position = UDim2.new(0.5, -75, 0, 38)
    ...
    ToggleButton.Parent = FlyToggleGui
    ...
    local function updateVisual()
        if flying then
            ...
            ToggleButton.TextColor3 = Color3.fromRGB(80, 255, 90)
            ToggleButton.BackgroundColor3 = Color3.fromRGB(30, 70, 35)
        else
            ToggleButton.Text = "OFF"
            ToggleButton.TextColor3 = Color3.fromRGB(255, 60, 60)
            ToggleButton.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    ...
    -- === DRAG SYSTEM (MÉTODO MAIS ESTÁVEL) ===
    ...
    TitleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            dragStart = input.Position
            startPos = FlyToggleGui.Position
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            FlyToggleGui.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)
...
local function destroyFloatingFlyToggle()
    if FlyToggleGui then
        FlyToggleGui:Destroy()
        FlyToggleGui = nil
    end
    stopFly()
end
...
Player.CharacterAdded:Connect(function()
    ...
    if FlyToggleGui then
        local btn = FlyToggleGui:FindFirstChild("ToggleBtn", true)
        if btn then
            btn.Text = "OFF"
